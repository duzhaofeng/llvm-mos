...
/// 7-bit data space address fixup for the LDS/STS instructions on MCS51Tiny.
///
/// Resolves to:
/// 1010 ikkk dddd kkkk
...
void MCS51AsmBackend::adjustFixupValue(const MCFixup &Fixup,
  ...
  case MCS51::fixup_hh8_ldi_pm:
    if (Kind == MCS51::fixup_hh8_ldi_pm)
      adjust::pm(Value);
  ...
  case MCS51::fixup_lo8_ldi_pm_neg:
    if (Kind == MCS51::fixup_lo8_ldi_pm_neg)
      adjust::pm(Value);
  ...
  case MCS51::fixup_hi8_ldi_pm_neg:
    if (Kind == MCS51::fixup_hi8_ldi_pm_neg)
      adjust::pm(Value);
  ...
  case MCS51::fixup_hh8_ldi_pm_neg:
    if (Kind == MCS51::fixup_hh8_ldi_pm_neg)
      adjust::pm(Value);
...
std::optional<MCFixupKind> MCS51AsmBackend::getFixupKind(StringRef Name) const {
  unsigned Type;
...
bool MCS51AsmBackend::writeNopData(raw_ostream &OS, uint64_t Count,
                                 const MCSubtargetInfo *STI) const {
