...
#define PASS_NAME "MCS51 DAG->DAG Instruction Selection"
...
unsigned MCS51DAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD, MVT VT,
                                                   int Bank) {
...
template <> bool MCS51DAGToDAGISel::select<ISD::FrameIndex>(SDNode *N) {
  ...
  CurDAG->SelectNodeTo(N, MCS51::FRMIDX, getTargetLowering()->getPointerTy(DL),
...
template <> bool MCS51DAGToDAGISel::select<ISD::LOAD>(SDNode *N) {
  ...
  int ProgMemBank = MCS51::getProgramMemoryBank(LD);
  ...
  if (unsigned LPMOpc = selectIndexedProgMemLoad(LD, VT, ProgMemBank)) {
    ...
    if (ProgMemBank == 0) {
      ...
      auto *NP = CurDAG->getMachineNode(MCS51::LDIRdK, DL, MVT::i8, NC);
    ...
    case MVT::i8:
      if (ProgMemBank == 0) {
        unsigned Opc = Subtarget->hasLPMX() ? MCS51::LPMRdZ : MCS51::LPMBRdZ;
        ResNode =
        ...
        SDValue NC = CurDAG->getTargetConstant(ProgMemBank, DL, MVT::i8);
        auto *NP = CurDAG->getMachineNode(MCS51::LDIRdK, DL, MVT::i8, NC);
        ResNode = CurDAG->getMachineNode(MCS51::ELPMBRdZ, DL, MVT::i8, MVT::Other,
                                         Ptr, SDValue(NP, 0));
      }
      break;
    case MVT::i16:
      if (ProgMemBank == 0) {
        ResNode =
            CurDAG->getMachineNode(MCS51::LPMWRdZ, DL, MVT::i16, MVT::Other, Ptr);
      } else {
        // Do not combine the LDI instruction into the ELPM pseudo instruction,
        // since LDI requires the destination register in range R16~R31.
        SDValue NC = CurDAG->getTargetConstant(ProgMemBank, DL, MVT::i8);
        auto *NP = CurDAG->getMachineNode(MCS51::LDIRdK, DL, MVT::i8, NC);
        ResNode = CurDAG->getMachineNode(MCS51::ELPMWRdZ, DL, MVT::i16,
...
template <> bool MCS51DAGToDAGISel::select<MCS51ISD::CALL>(SDNode *N) {
  ...
  SDLoc DL(N);
  Chain = CurDAG->getCopyToReg(Chain, DL, MCS51::R31R30, Callee, InGlue);
  ...
  SDNode *ResNode = CurDAG->getMachineNode(
      Subtarget->hasEIJMPCALL() ? MCS51::EICALL : MCS51::ICALL, DL, MVT::Other,
      MVT::Glue, Ops);
