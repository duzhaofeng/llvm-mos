...
void MCS51InstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (MCS51::DREGSRegClass.contains(DestReg, SrcReg)) {
    // If our MCS51 has `movw`, let's emit that; otherwise let's emit two separate
    // `mov`s.
...
Register MCS51InstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
...
Register MCS51InstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                          int &FrameIndex) const {
  switch (MI.getOpcode()) {
...
unsigned MCS51InstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  ...
  case TargetOpcode::INLINEASM_BR: {
    ...
    const MCS51TargetMachine &TM =
        static_cast<const MCS51TargetMachine &>(MF.getTarget());
...
void MCS51InstrInfo::insertIndirectBranch(MachineBasicBlock &MBB,
  ...
  if (STI.hasJMPCALL())
    BuildMI(&MBB, DL, get(MCS51::JMPk)).addMBB(&NewDestBB);
  else
    ...
    BuildMI(&MBB, DL, get(MCS51::RJMPk)).addMBB(&NewDestBB);
}
